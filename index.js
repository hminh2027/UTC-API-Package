...
export const getAllSchedule = async (username, password) => {
    ...
    try {
        const rs = await axios.post('http://utc-schedule-api.herokuapp.com/schedule', {
            username,
            password
        })
...
export const getScheduleOfToday = async (username, password) => {
    if (!username || !password) return {data: '', error: 'Username or password not found!'}
    try {
        const rs = await axios.post('http://utc-schedule-api.herokuapp.com/schedule/today', {
            username,
            password
        })
...
export const getScheduleOfDay = async (username, password, date) => {
    ...
    try {
        const rs = await axios.post(`http://utc-schedule-api.herokuapp.com/schedule/day?year=${date.getFullYear()}&month=${date.getMonth()}&day=${date.getDate()}`, {
            username,
            password
        })
